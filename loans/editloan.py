...
class EditLoanDlg(gtk.Dialog):
    def __init__(self, parent, model, iter):
        ...
        entry = gtk.Entry()
        entry.set_text(self.model.get_value(self.iter, 1))
        ...
        entry = gtk.Entry()
        entry.set_text(self.model.get_value(self.iter, 2))
        self.elements.append(entry)
        ...
        entry = gtk.Entry()
        entry.set_text("%d"%self.model.get_value(self.iter, 3))
        self.elements.append(entry)
        ...
        check.set_active(self.model.get_value(self.iter, 4))
        ...
        check = gtk.CheckButton()
        check.set_active(self.model.get_value(self.iter, 5))
        self.elements.append(check)
        ...
        textview.get_buffer().set_text(self.model.get_value(self.iter, 6))
    ...
    def check(self, widget):
        ...
        self.model.update(self.iter, \
                          self.model.get_value(self.iter, 0), \
                          self.elements[0].get_text(), \
...
if __name__ == "__main__":
    ...
    model = gtk.ListStore(int, str, str, float, bool, bool, str)
    model.append([1,"Nokia 2630","1234567890",1000,True,False,"česky"])
    model.edit = edit
    
    dlg = EditLoanDlg(None, model, model.get_iter_first())
    res = dlg.run()
